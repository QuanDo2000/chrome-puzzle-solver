...
function spies() {
  ...
  return { calls, applySolution, dispatchApplyHint };
}

test('applyLoopStep: a full-grid puzzle applies the read-back grid', async () => {
  const { calls, applySolution, dispatchApplyHint } = spies();
  const grid = [[0, 1], [2, 0]];
  const ar = await applyLoopStep({}, { type: 'tents', cells: [] }, grid, { applySolution, dispatchApplyHint });
  assert.deepEqual(ar, { success: true });
  assert.deepEqual(calls, [['applySolution', grid]]);
});

test('applyLoopStep: a loopApplyViaHint puzzle applies only the hint delta, never the full grid', async () => {
  const { calls, applySolution, dispatchApplyHint } = spies();
  const grid = [[0, 0], [0, 0]];
  const hint = { type: 'shakashaka', extraCells: [{ row: 0, col: 0, value: 1 }] };
  const ar = await applyLoopStep({ loopApplyViaHint: true }, hint, grid, { applySolution, dispatchApplyHint });
  assert.deepEqual(ar, { success: true });
  assert.deepEqual(calls, [['dispatchApplyHint', hint]]);
  // crucially, the full grid is NEVER applied — that is what over-commits
  // every undecided cell to white.
  assert.ok(!calls.some(c => c[0] === 'applySolution'), 'full grid must not be applied');
});

test('applyLoopStep: a galaxies hint applies its computed lines regardless of flag', async () => {
  const { calls, applySolution, dispatchApplyHint } = spies();
  const ar = await applyLoopStep({}, { type: 'galaxies', lines: ['L'] }, [], { applySolution, dispatchApplyHint });
  assert.deepEqual(ar, { success: true });
