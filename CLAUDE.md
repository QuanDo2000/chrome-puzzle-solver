...
helpers (`hashFNV1a` in each, plus the solver layer's `emitGrid`,
`cloneSolveResult`, `timeUp`, `lruSet`, `whiteConnectivity`,
`trailPush`/`rollbackTrail`, `collectChangedCells`; and the widget layer's
`drawCrossCell`, `absoluteCellHintStatus`, `makeSimpleHintDispatch`). Consumers
`require('./shared.js')` (or
...
- Shingoki — `src/widget/puzzles/shingoki.js`, `src/solvers/shingoki.js` (Hint/Loop use a deductive getStepwiseHint — border/axis + number-run propagation + 1-step lookahead — falling back to the cached solution when logic is exhausted) (solver: adaptive DFS + two-tier deduction engine (_deduceAll): Tier 1 = _propagate + _maxReachForce; Tier 2 (_deduceHeavy) = _candidateIntersectForce + _connectivityForce + _bifurcateForce. Measured reach/solve on real boards: 7x7-hard solves ~0.4s (reach 39/112); 10x10-hard solves ~1.7s (reach 11/220); 15x15-hard solves ~25s with large budget — sound partial at 6s default (reach 66/480); 25x25-hard returns a sound partial even at 30s (reach 79/1300). CDCL was tried and removed: useless+harmful on connectivity-dominated boards).
