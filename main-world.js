...
function applyStarBattleState(solution) {
  try {
    ...
    if (!(G && G.currentState)) return false;
    if (typeof G.saveState === 'function') G.saveState(true);
    // Solution/apply value-space: 1 star, 2 X (no-star marker), 0 empty, 9 UNK. Write stars and
    // X-marks; leave 0/9 untouched so an empty board stays empty and a partial only commits the
    // cells it actually deduced.
